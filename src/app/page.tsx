      ...
      <div className="max-w-[1400px] mx-auto px-6 md:px-12">
        ...
        <div className="group/pricing">
            ...
            {plans.map((p, i) => {
              ...
              return (
                <motion.div key={p.name} initial={{ opacity: 0, y: 60 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-10%" }} transition={{ delay: i * 0.1, duration: 0.6, ease: REVEAL }}
                  className={`relative rounded-2xl border p-6 flex flex-col transition-colors duration-300 ${p.highlight ? "border-2 border-white shadow-lg bg-white text-neutral-950" : "border-neutral-700 bg-neutral-800/50 group-hover/pricing:border-neutral-400 group-hover/pricing:bg-neutral-700/60 hover:bg-white hover:border-white hover:shadow-lg"}`}>
                  {p.highlight && (
                  ...
                  <div className={p.highlight ? "pt-2" : ""}>
                    <h3 className={`text-lg font-semibold mb-1 ${p.highlight ? "" : "text-white group-hover/pricing:text-neutral-100 hover:text-neutral-950"}`}>{p.name}</h3>
                    <p className={`text-sm mb-4 ${p.highlight ? "text-neutral-500" : "text-neutral-400 group-hover/pricing:text-neutral-300 hover:text-neutral-500"}`}>{p.desc}</p>
                    <div className="mb-4">
                      <span className={`text-3xl font-bold ${p.highlight ? "" : "text-white group-hover/pricing:text-white hover:text-neutral-950"}`}>{price.toLocaleString("ru-RU")}</span>
                      <span className={`${p.highlight ? "text-neutral-500" : "text-neutral-400 group-hover/pricing:text-neutral-300 hover:text-neutral-500"}`}> BYN/мес</span>
                    </div>
                    <div className="space-y-2 mb-6 text-sm">
                      <div className="flex items-center gap-2"><CircleCheckBig className={`w-4 h-4 flex-shrink-0 ${p.highlight ? "text-green-600" : "text-green-500 group-hover/pricing:text-green-400 hover:text-green-600"}`} /><span className={`${p.highlight ? "text-neutral-700" : "text-neutral-300 group-hover/pricing:text-neutral-200 hover:text-neutral-700"}`}>{p.cap}</span></div>
                      <div className="flex items-center gap-2"><CircleCheckBig className={`w-4 h-4 flex-shrink-0 ${p.highlight ? "text-green-600" : "text-green-500 group-hover/pricing:text-green-400 hover:text-green-600"}`} /><span className={`${p.highlight ? "text-neutral-700" : "text-neutral-300 group-hover/pricing:text-neutral-200 hover:text-neutral-700"}`}>Весь функционал</span></div>
                    </div>
                  </div>
                  <div className="mt-auto">
                    <a href="/register" className={`block text-center w-full py-2.5 rounded-xl text-sm font-medium transition-colors ${p.highlight ? "bg-neutral-950 text-white hover:bg-neutral-800" : "border border-neutral-600 text-neutral-200 group-hover/pricing:border-neutral-400 hover:bg-neutral-950 hover:text-white hover:border-neutral-950"}`}>Выбрать</a>
                  </div>
                </motion.div>
              );
            })}
          ...
          <div className="text-center">
            <a className="inline-flex items-center gap-2 text-sm text-neutral-400 hover:text-white transition-colors group-hover/pricing:text-teal-300" href="/verticals/security/pricing">Подробнее о тарифах<ArrowRight className="w-4 h-4" /></a>
          </div>
        </div>
      </div>
    </section>
